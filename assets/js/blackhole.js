...
(function () {
  ...
  var FRAG = [
    ...
    '',
    '  /* Lensed image of the far disk: a hot arc bent over the hole */',
    '  float arc = exp(-pow((rc - 0.185) * 34.0, 2.0)) * smoothstep(0.0, 0.55, uv.y / max(rc, 0.001));',
    ...
    '',
    '  /* Relativistic color shift: blue-white approaching, reddened receding */',
    ...
    '  c += mix(vec3(1.0, 0.85, 0.55), vec3(1.0, 0.97, 0.9), 0.5) * arc * 0.75;',
    '  /* soft bloom hugging the photon ring */',
    '  c += vec3(1.0, 0.8, 0.5) * exp(-pow((rc - 0.148) * 13.0, 2.0)) * 0.16;',
    '',
    '  /* Sparse twinkling starfield outside the disk */',
    ...
    '  c += vec3(0.75, 0.82, 1.0) * star * 1.4;',
    '',
    '  /* Faint ambient halo so the hole reads against the page */',
    '  c += ember * 0.045 * (1.0 - smoothstep(0.0, 0.9, rc));',
    '',
    '  c *= 1.0 - horizon;',
    '  float alpha = clamp(bright * 1.5 + photon * 0.8 + arc * 0.6 + star * 1.2 + exp(-pow((rc - 0.148) * 13.0, 2.0)) * 0.12 + 0.04 * (1.0 - smoothstep(0.0, 0.9, rc)), 0.0, 1.0);',
